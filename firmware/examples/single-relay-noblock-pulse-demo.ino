...
RelayLib relay(relay_pin);  // still requires init() to set pinMode to output

void setup()
{
  Serial.begin(9600);
  relay.init();  // BEGIN: pin, Initial State (HIGH or LOW), Relay Action (ACTIVE_LOW or ACTIVE_HIGH), Pulse Delay in mS
  relay.on();
  delay(2000);
  relay.off();
  delay(2000);
